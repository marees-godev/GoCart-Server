...
// metadata (Request ID, User ID, tracing), and enforces timeout deadlines.
func UnaryClientInterceptor(defaultTimeout time.Duration) grpc.UnaryClientInterceptor {
	return func(
		...
		// 2. Enforce deadline/timeout if none set or if default is smaller
		...
		if err != nil {
			logger.FromContext(ctx).Warn("Downstream gRPC call failed",
				"method", method,
				"target", cc.Target(),
